...
def train_model_logic(df: pd.DataFrame, problem_type: str, algorithm: str, target_col: str = None):
    ...
    if problem_type in ('classification', 'regression'):
        ...
        # ── AUTO-FILTER: Only use numeric columns for training ──
        # Any string/object column NOT encoded will be dropped to prevent errors
        X = X.select_dtypes(include=[np.number])
        
        if X.empty:
            raise ValueError("No numeric features found for training. Please encode your text columns first!")
